...
exports.earnPickupPoints = exports.earnCashback = exports.getRewardTiers = exports.getRedemptionHistory = exports.redeemRewards = exports.useReferralCode = exports.getUserReferrals = exports.getUserRewards = void 0;
const prisma_1 = require("../utils/prisma");
const zod_1 = require("zod");
const crypto_1 = __importDefault(require("crypto"));
const RewardPointTypeEnum = zod_1.z.enum(['PICKUP', 'PAYMENT', 'REFERRAL_SIGNUP', 'REFERRAL_BONUS', 'CASHBACK', 'BONUS', 'REDEMPTION', 'PICKUP_COMPLETE']);
const RedemptionStatusEnum = zod_1.z.enum(['PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED']);
const RewardRedemptionTypeEnum = zod_1.z.enum(['AIRTIME', 'WALLET_CREDIT', 'DISCOUNT_CODE', 'UTILITY_BILL']);
const useReferralSchema = zod_1.z.object({
    referralCode: zod_1.z.string().min(6).max(20)
});
const redeemRewardSchema = zod_1.z.object({
...
const getUserRewards = async (req, res, next) => {
    try {
        ...
        // Get total points
        const pointsAggregation = await prisma_1.prisma.rewardPoint.aggregate({
            where: { userId },
            _sum: { points: true }
        });
        const totalPoints = pointsAggregation._sum.points || 0;
        // Get recent transactions
        const recentTransactions = await prisma_1.prisma.rewardPoint.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
            take: 10
        });
        // Get current tier
        const tiers = await prisma_1.prisma.rewardTier.findMany({
            where: { isActive: true },
...
const getUserReferrals = async (req, res, next) => {
    try {
        ...
        let referral = await prisma_1.prisma.referral.findFirst({
            where: { referrerId: userId }
        });
        if (!referral) {
            const code = generateReferralCode();
            referral = await prisma_1.prisma.referral.create({
                data: {
                    referrerId: userId,
                    referralCode: code
                }
        ...
        // Get referral stats
        const totalReferrals = await prisma_1.prisma.referral.count({
            where: { referrerId: userId }
        });
        const completedReferrals = await prisma_1.prisma.referral.count({
        ...
        const referralEarnings = await prisma_1.prisma.rewardPoint.aggregate({
            where: {
                userId,
...
const useReferralCode = async (req, res, next) => {
    try {
        ...
        await prisma_1.prisma.rewardPoint.create({
            data: {
...
const redeemRewards = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        ...
        switch (validatedData.rewardType) {
            case 'AIRTIME':
                ...
                const validProvider = AIRTMIME_PROVIDERS.find(p => p.codes.includes(provider?.toLowerCase()));
                if (!validProvider) {
                    return res.status(400).json({
        ...
        const redemption = await prisma_1.prisma.rewardRedemption.create({
            data: {
                userId,
        ...
        if (validatedData.rewardType === 'WALLET_CREDIT') {
            ...
            await prisma_1.prisma.rewardRedemption.update({
...
const getRedemptionHistory = async (req, res, next) => {
    try {
        ...
        const [redemptions, total] = await Promise.all([
            prisma_1.prisma.rewardRedemption.findMany({
                where,
                orderBy: { createdAt: 'desc' },
                skip,
                take: Number(limit)
            }),
            prisma_1.prisma.rewardRedemption.count({ where })
        ]);
        res.json({
...
const earnCashback = async (userId, paymentAmount) => {
    ...
    const totalPoints = pointsAggregation._sum.points || 0;
    const currentTier = await prisma_1.prisma.rewardTier.findFirst({
        where: {
            isActive: true,
...
exports.earnCashback = earnCashback;
const earnPickupPoints = async (userId, pickupId) => {
...
exports.earnPickupPoints = earnPickupPoints;
// Helper functions
...
function generateDiscountCode() {
    return `ECO${crypto_1.default.randomBytes(4).toString('hex').toUpperCase()}`;
}
