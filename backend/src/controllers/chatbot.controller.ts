...
const AI_RESPONSES: Record<string, { keywords: string[]; response: string; followUp?: string }[]> = {
  greeting: [
    {
...
async function findAIResponse(message: string): Promise<{ response: string; followUp?: string }> {
...
async function findFAQResponse(lowerMessage: string): Promise<{ response: string; followUp?: string }> {
  try {
...
function getContextualSuggestions(conversationHistory: any[]): string[] {
  ...
  if (conversationHistory.length === 0) {
    ...
    if (!hasPricing) suggestions.push('View pricing');
    if (!hasSchedule) suggestions.push('Book a pickup');
    ...
    if (!conversation) {
      ...
      conversation = await prisma.chatConversation.create({
        data: {
          ...
          sessionId: validatedData.sessionId || crypto.randomBytes(16).toString('hex'),
          status: 'ACTIVE'
        },
        include: { messages: { orderBy: { createdAt: 'asc' } } }
      });
